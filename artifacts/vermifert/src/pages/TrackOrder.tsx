...
export default function TrackOrder() {
  ...
  // Support /track/VF2024ABCDEF URL pattern
  ...
  async function doTrack(tn: string) {
    ...
    try {
      ...
      if (!res.ok) { setError(data.error ?? "رقم التتبع غير صحيح"); return; }
      setResult(data);
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1 container mx-auto px-4 py-12 max-w-2xl">
        <div className="text-center mb-10">
          ...
          <p className="text-muted-foreground mt-1 text-sm">أدخل رقم التتبع للاطلاع على حالة طلبك</p>
        </div>
        ...
        {result && (
          <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm">
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <div className="text-sm text-muted-foreground flex flex-wrap gap-x-4 gap-y-0.5">
                    <span>الكمية: {result.quantity}</span>
                    <span>{result.totalPrice.toLocaleString("ar-DZ")} د.ج</span>
                    <span>{result.city}</span>
                    <span>{format(new Date(result.createdAt), "d MMMM yyyy", { locale: ar })}</span>
                  </div>
                </div>
            ...
            <div className="px-5 py-6">
              <h3 className="text-sm font-semibold text-muted-foreground mb-4">مراحل التوصيل</h3>
